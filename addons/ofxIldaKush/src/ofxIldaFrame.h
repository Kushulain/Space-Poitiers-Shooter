    ...
    public:
        ...
        struct {
            ...
            struct {
                ...
                int pointLineCount;       // how many end repeats to send per line
        ...
        void setDefaultParams() {
            ...
            params.output.pointLineCount = 5;
        ...
        string getString() {
            ...
            s << "output.pointLineCount : " << params.output.pointLineCount << endl;
            ...
            s << "trapezeX : " << trapezeX << endl;
            s << "trapezeY : " << trapezeY << endl;
            s << "rot : " << rot << endl;
        ...
        ofPoint transformPoint(ofPoint p) const {
            ...
            if(params.output.transform.scale.lengthSquared() > 0) {
                ...
                p = p.x * ofVec2f(rotMat[0],rotMat[1]) + p.y * ofVec2f(rotMat[2],rotMat[3]); //rotate
                p.x *= 1.0f + trapezeX * p.y;
                p.y *= 1.0f + trapezeY * p.x;
                p += ofPoint(0.5, 0.5);
            }
        ...
        void updateFinalPoints() {
            ...
            rotMat[0] = cos(radAngleRight);
            rotMat[1] = sin(radAngleRight);
            rotMat[2] = cos(radAngleForward);
            rotMat[3] = sin(radAngleForward);
            ...
            for(int i=0; i<processedPolys.size(); i++) {
                ...
                if(poly.size() > 0) {
                    ...
                    for(int j=0; j<poly.size(); j++) {
                      Point myP = Point(transformPoint(poly[j]), pcolor);
                      if (processedPolys[i].pointsPerUnit == -1.0)
                      {
                        for (int o=0; o<params.output.pointLineCount; o++)
                        {
