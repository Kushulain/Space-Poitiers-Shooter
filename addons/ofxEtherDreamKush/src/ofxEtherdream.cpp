...
void ofxEtherdream::init() {
	...
	for (int i=0; i<device_num; i++) {
		...
		if (etherDreamIP == inet_ntoa(etherdream_get(i)->addr))
		{
			ofLogNotice() << etherDreamIP;
			ofLogNotice() << inet_ntoa(etherdream_get(i)->addr);
			ofLogNotice() << "MATCH";
			idEtherdreamConnection = i;
		}
  }
    ...
    ofLogNotice() << inet_ntoa(device->addr);
